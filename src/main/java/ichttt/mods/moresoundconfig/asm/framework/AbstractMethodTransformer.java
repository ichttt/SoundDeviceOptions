...
public abstract class AbstractMethodTransformer implements IClassTransformer, Opcodes {
    ...
    private byte[] findMethodAndPatch(String name, String transformedName, byte[] basicClass) {
        MSCCoreMod.LOGGER.info("Starting patch of class {}", name);
        ...
        if (!found) {
            MSCCoreMod.LOGGER.fatal("Did not find method (mcp: {} srg: {}) with desc {}", mcpName, srgName, methodDesc);
        ...
        MSCCoreMod.LOGGER.info("Patch of class {} ended", name);
        return data;
    }
